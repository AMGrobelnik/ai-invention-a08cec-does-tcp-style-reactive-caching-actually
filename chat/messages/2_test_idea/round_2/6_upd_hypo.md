...
### [1] CONFIG · 2026-08-10 04:55:00 UTC

```
model: claude-sonnet-5 | effort: low | permission: bypassPermissions | cwd: /ai-inventor/aii_data/runs/run_MmmgOkQFZ5uI/3_invention_loop/iter_2/upd_hypo/upd_hypo
```

### [2] SYSTEM-USER prompt · 2026-08-10 04:55:06 UTC
...
### [3] HUMAN-USER prompt · 2026-08-10 04:55:07 UTC
...
### [4] TOOL CALL — Write · 2026-08-10 04:56:31 UTC

```
File: /ai-inventor/aii_data/runs/run_MmmgOkQFZ5uI/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json

{
  ...
  "motivation": "The strongest existing adaptive-TTL result, d-TTL/f-TTL from CDN caching research, provably converges a TTL parameter toward a target *hit rate* using stochastic approximation -- but hit rate says nothing about whether those hits returned correct, non-stale content; it was built for content-delivery traffic where staleness usually isn't safety-relevant. The strongest existing staleness-aware caches for LLM pipelines, FreshCache (risk-constrained temporal caching for RAG) and vCache (verified semantic caching with Bayesian per-entry error-rate guarantees), instead fit an explicit probabilistic staleness/error model per entry and gate reuse against a fixed error budget -- powerful, but they require a labeled calibration signal to fit that model before the guarantee is meaningful, and the model itself must be re-fit or re-trusted whenever workload volatility shifts. Agent loops were hypothesized to be exactly the setting where a third option -- a cheap, purely reactive, feedback-driven AIMD rule with no fitting step -- would be attractive precisely because many call sites are visited only a handful of times per episode. Two iterations of increasingly careful measurement (fixing a silent dataset-wiring bug, adding independent bootstrap statistics, and now flagging an asymmetric knob-grid and a missing repeat-count breakdown) have not yet found that advantage: on real content specifically, a fitted gate's point estimate wins on hit rate at comparable staleness, and AIMD is confirmed slower, not faster, to reach a stable operating point. The remaining open question this hypothesis now tracks is narrower and more falsifiable than before: does AIMD's real-data disadvantage survive (i) statistically rigorous confidence intervals and (ii) a fair, matched knob-search budget against FreshCache, and does it specifically survive restriction to the low-repeat-count regime where a fitted model has the least data to work with -- the one regime where AIMD's model-free reactivity should, in principle, have room to matter.",
  "assumptions": [
    ...
    "A simulated or replayed agent-loop workload with realistic call repetition and controllable underlying volatility (so ground-truth staleness events can be injected/observed) is constructible from real seed content (Wikipedia/SQuAD, Quora Question Pairs, Our World in Data) replayed through deterministic repetition templates, since real production agent traces are not directly accessible -- though this remains a proxy for actual agent behavior, not a substitute for it",
    ...
    "A knob-search budget that is matched (or explicitly justified as saturating) across all compared policy families is necessary for a dominance claim between them to be trustworthy -- an asymmetric grid (9 points for AIMD vs 3-5 for baselines) is not a neutral simplification and must be corrected or justified before the real-data dominance finding can be treated as decisive"
  ],
  "investigation_approach": "Build an agent-loop tool-call harness (real-content-grounded versioned resource corpus replayed through deterministic repetition templates, or an OpenRouter-backed live-agent variant as a stretch goal) that logs every tool call, arguments, timestamp, and result, and can force a live re-query or ground-truth version-schedule lookup on a sampled subset of cache hits to get a binary valid/stale label. Implement cache policies at each call site: (1) fixed TTL swept over several values; (2) a d-TTL-style stochastic-approximation policy and an EWMA-adaptive policy that adapt TTL toward a target hit rate; (3) the proposed AIMD policy -- reuse window w_i per call site i, additive increase after each confirmed-valid hit, multiplicative decrease after each confirmed-stale hit, with w_i floor/ceiling bounds; and (4) a FreshCache-style fitted staleness-probability gate (raw and resource-class-pooled variants) as an upper-reference. Critically, next iteration must: (i) run the already-built bootstrap-CI/Holm-correction evaluation machinery against the real-data replicate rows (20 seeds/cell already exist) to attach confidence intervals and significance tests to the real-data dominance claim, not just the synthetic re-derivation; (ii) widen the real-data baseline knob grids to match AIMD's 9-point grid, or explicitly justify why the narrower grids already saturate each baseline's achievable frontier; (iii) break out Table-1-style hit-rate/stale-rate figures by call-site repeat-count bucket (low-repeat <=5 visits, matching the existing convergence-analysis bucketing) to test whether FreshCache's real-data dominance holds, narrows, or reverses specifically where a fitted model has the least data to calibrate on. Continue reporting the previously-established convergence-event and spot-check-density-ablation findings as settled (they are now confidence-interval-backed and mechanistically explained), and scope any novelty claims explicitly to caching literature unless an additional adjacent-domain (rate-limiting/autoscaling) search angle is added.",
  "success_criteria": "CONFIRMS the original hypothesis's frontier claim only if, once the real-data dominance result is independently bootstrap-CI'd and significance-tested with a matched (or justified) knob-search budget across all policy families, AIMD's frontier position is NOT statistically dominated by FreshCache in the low-repeat-count bucket specifically (the regime the hypothesis's mechanism argues should favor a model-free reactive rule). DISCONFIRMS/refutes the frontier claim if, under those corrected conditions, FreshCache's dominance survives statistically and holds (or worsens) in the low-repeat bucket -- this would mean the fitted-gate approach beats AIMD even in exactly the regime AIMD was expected to have an edge, a strong and specific negative result. The convergence-speed claim (criterion b) is treated as already DISCONFIRMED with reasonable confidence given two independent lines of evidence (this iteration's CI-backed re-derivation and the mechanistic spot-check-density ablation); it is retained in the hypothesis only as a settled negative finding to report, not as an open question for further testing, unless a modified AIMD schedule (e.g., decaying additive increase, as noted in future work) is tested and shown to close the gap.",
  "related_works": [
    "FreshCache: Risk-Constrained Freshness-Aware Semantic Caching for Open-Web RAG (Mansoor, Ahmad & Yoon, arXiv:2607.04281, 2026) -- read in full. Fits an exponential-decay-plus-MLP staleness-probability model per entry/tier and gates reuse against a fixed per-tier error budget (0.10/0.20/0.35), evaluated on 8,072-31,201 real web queries with ground-truth snapshot labels. This is the closest prior mechanism -- probabilistic risk-budget gating -- and this iteration's real-data replay found it Pareto-dominates AIMD as a point estimate on real content, though that dominance is not yet independently statistically confirmed and has not been checked in the low-repeat-count regime where FreshCache's >=5-observation fitting requirement should be weakest.",
    ...
    "Adaptive TTL-Based Caching for Content Delivery -- d-TTL and f-TTL (Basu, Sundarrajan, Ghaderi, Shakkottai & Sitaraman, arXiv:1704.04448 / IEEE, 2017) -- read in full. d-TTL uses stochastic approximation (actor-critic style) to converge a per-object TTL toward a *target hit rate*, with provable convergence for bursty non-stationary CDN traffic. Neither d-TTL nor its EWMA-adaptive variant optimizes for confirmed correctness/staleness, and on the real-content replay both were matched or beaten by AIMD on hit rate at comparable staleness -- the frontier gap that remains is specifically against the fitted FreshCache gate, not against hit-rate-targeted adaptation.",
    ...
    "TVCACHE: A Stateful Tool-Value Cache for Post-Training LLM Agents (Vijaya Kumar et al., arXiv:2602.10986, 2026) -- caches by exact longest-prefix match on the agent's full tool-call-history tree, aimed at RL post-training rollouts with high trajectory overlap; this is an exact-match structural cache with no notion of graded time-based staleness or per-entry reuse window at all, addressing a different regime than the present hypothesis's within-episode, non-identical-trajectory redundant calls.",
    "Concur: Proactive Agent-Level Admission Control for Efficient Agentic Batch Inference (Chen et al., ICML 2025, arXiv:2601.22705) -- applies genuine two-sided AIMD inside an LLM-agent inference-serving system, but to admission control of concurrent agents gated on KV-cache pressure, never to any object's TTL or content freshness; establishes AIMD as a live pattern in this research community for a categorically different control variable.",
    "ClepsydraCache (Thoma et al., USENIX Security 2023, arXiv:2104.11469) and the Alex filesystem / RFC 7234 heuristic freshness rule -- the two closest non-agent, non-LLM near-hits for AIMD-shaped or age-proportional TTL adaptation, confirmed via a systematic literature search across CDN, database, DNS, and browser/HTTP caching literature to be triggered by security conflicts or resource age respectively, not confirmed content staleness; no hit was found applying a loss-event-triggered AIMD rule to per-object TTL in any of these domains, though the search did not extend to adjacent control-theoretic domains (rate-limiting, autoscaling/admission-control) where AIMD-style window adaptation is also an established pattern."
  ],
  "inspiration": "PROCEDURAL/METHODOLOGICAL: TCP congestion control's AIMD rule was designed for exactly this class of problem -- adapt a resource-usage window under noisy, sparse, delayed feedback about an unknown and shifting environment, without ever fitting an explicit model of that environment, and recover fast from a bad outcome (packet loss / here, a confirmed-stale serve) via a sharp multiplicative cut while probing for more headroom via slow additive growth when things go well. That asymmetry -- cheap to gain, expensive to lose -- remains the shape wanted for cache reuse windows once staleness is reframed as a 'loss event', even though two iterations of increasingly rigorous measurement have not yet found it beating a fitted probabilistic gate. What now motivates continuing the investigation, rather than abandoning it, is a genuinely open, narrower question: the gap so far is measured only as a point estimate, with an unfair knob-search budget, and pooled across all repeat-count regimes including ones where a fitted model should have every advantage -- the fair, statistically confirmed, low-repeat-specific comparison has simply not been run yet.",
  "terms": [
    {
      "term": "AIMD (additive increase, multiplicative decrease)",
      "definition": "A feedback control rule, the core of TCP congestion control, that slowly grows a resource window on success and sharply shrinks it on a detected failure/overload signal, converging toward an efficient operating point without needing a model of the underlying system."
    },
    {
      "term": "Cache reuse window",
      "definition": "The time interval, maintained per tool-call site, during which a cached tool result is served instead of re-executing the call; here it grows or shrinks based on confirmed-valid or confirmed-stale outcomes rather than being fixed or fit from a probability model."
    },
    {
      "term": "Confirmed-stale event",
      "definition": "An observed instance where a cache hit was served but a live re-query (or later ground truth) shows the cached value no longer matches -- the 'loss signal' the AIMD rule reacts to, analogous to a dropped packet in TCP."
    },
    {
      "term": "Reduction-vs-staleness frontier",
      "definition": "The tradeoff curve between how many redundant tool calls a caching policy avoids (efficiency) and how often it serves an outdated answer (correctness risk); comparing policies means comparing where each one lands on, and how it sweeps, this curve."
    },
    {
      "term": "Point-estimate dominance vs. statistically confirmed dominance",
      "definition": "A distinction introduced this iteration: one policy's mean metrics beating another's across replicate seeds (point-estimate dominance) is not the same claim as that gap surviving bootstrap confidence intervals and significance testing (statistically confirmed dominance) -- the real-data FreshCache-over-AIMD result is currently only the former."
    },
    {
      "term": "Low-repeat-count bucket",
      "definition": "Call sites visited five or fewer times per episode, matching the real corpus's own median revisit count; the regime the hypothesis identifies as both most representative of real agent-tool-call traffic and most disadvantageous for a fitted model like FreshCache that needs several observations to calibrate."
    }
  ],
  "summary": "We hypothesized that a TCP-congestion-control-style AIMD rule for per-call-site cache reuse windows matches or beats both fixed TTL and hit-rate-targeted adaptive TTL on the reduction-vs-staleness tradeoff while converging faster than a fitted probabilistic staleness gate. Two iterations of increasingly rigorous measurement instead found AIMD's convergence-speed advantage confidently disconfirmed (CI-backed, mechanistically explained by spot-check density) and its frontier position beaten by a fitted gate on real content -- but only as an unverified point estimate, with an unfair knob-search budget, and never yet checked in the specific low-repeat-count regime the mechanism was supposed to favor. The hypothesis now narrows to that unresolved, fair, statistically-confirmed, repeat-count-stratified comparison.",
  "relation_type": "evolution",
  "relation_rationale": "Same AIMD-vs-baselines frame; frontier claim narrowed to unverified-point-estimate + low-repeat gap per review",
  "confidence_delta": "decreased",
  "key_changes": [
    "Reframed the real-data dominance finding (Table 1) from an established result to explicitly unverified point-estimate dominance, per reviewer MAJOR feedback that it lacks the bootstrap CIs the paper's own Section 5 demands of others",
    "Added the asymmetric knob-search-budget caveat (9 points for AIMD vs 3-5 for baselines) as a first-class methodological gap that must be closed before the dominance claim is trusted, per reviewer MINOR feedback",
    "Added the missing low-repeat-count breakdown as the single most important next test, since the hypothesis's own mechanism argues AIMD should have its best chance exactly where FreshCache has the least data to fit on -- currently untested",
    "Retained and hardened the convergence-speed disconfirmation (now CI-backed across two independent artifacts plus a mechanistic spot-check-density ablation) as a settled negative finding rather than an open question",
    "Softened novelty-search language to scope explicitly to caching literature, per reviewer MINOR feedback that adjacent control-theoretic domains (rate-limiting, autoscaling) were not searched",
    "Investigation approach now prioritizes re-running existing evaluation machinery on existing real-data replicate rows (no new data collection needed) plus a matched-grid rerun, rather than proposing new experiment infrastructure"
  ],
  "artifact_relations": [
    {
      "from_id": "art_T0onLH9xokqw",
      "to_id": "art_tceB4eOwcBAO",
      "relation_type": "uses",
      "relation_rationale": "Loads the real-content versioned corpus via a fail-fast dependency loader for the real-data replay"
    },
    {
      "from_id": "art_qtEtMpLZuRGI",
      "to_id": "art_tXld0p2SGjtU",
      "relation_type": "extends",
      "relation_rationale": "Re-imports method.py, reproduces its simulator/seeds, and adds bootstrap CIs and Holm-corrected tests"
    },
    {
      "from_id": "art_T0onLH9xokqw",
      "to_id": "art_tXld0p2SGjtU",
      "relation_type": "uses",
      "relation_rationale": "Uses corpus version-schedule stats for the ecological-validity proxy comparison"
    }
  ]
}
```

### [5] TOOL RESULT — Write · 2026-08-10 04:56:31 UTC

```
{"type": "create", "filePath": "/ai-inventor/aii_data/runs/run_MmmgOkQFZ5uI/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json", "content": "{\n  \"title\": \"Congestion-Control-Style Cache TTL for Agents\",\n  \"hypothesis\": \"In an LLM agent loop, treating each tool-call site's cache lifetime as a TCP congestion window -- additively growing the reuse window after every confirmed-valid cache hit, and multiplicatively slashing it after every confirmed-stale hit -- was hypothesized to reach a comparable-or-better redundant-call-reduction-vs-stale-serve-rate operating point than (a) fixed TTL and (b) target-hit-rate stochastic-approximation TTL adaptation (d-TTL/EWMA), with a further advantage of needing far fewer confirmed-staleness feedback events than a fitted probabilistic staleness gate (FreshCache-style) to converge. Both claims are now REVISED DOWNWARD, and the evidence base itself has fractured into two lines that disagree with each other, which the hypothesis must now hold apart rather than average together. On a fresh, correctly-wired real-content replay (5,307 rows from Wikipedia/SQuAD, Quora Question Pairs, and Our World in Data, actually loaded this time via a fail-fast dependency loader), AIMD's full 9-point knob grid (hit rate 0.794-0.803, stale rate 0.106-0.109) is Pareto-dominated as a POINT ESTIMATE by a fitted staleness gate (FreshCache, hit rate 0.898-0.906 at comparable-or-lower stale rate) and indistinguishable as a point estimate from the much simpler EWMA-adaptive baseline (0.797-0.799 hit rate) -- but this real-data dominance finding has NOT been independently bootstrap-CI'd or significance-tested, unlike every other numeric claim in this line of work, and until it is, it must be reported and treated as unverified point-estimate dominance, not a confirmed result. Separately, an independent, bootstrap-CI'd, Holm-corrected re-derivation of the PRIOR iteration's synthetic-only run overturns that run's own self-reported frontier claim: mean AIMD non-dominated fraction collapses from a self-reported 0.67 to an independently re-derived 0.0 (CIs excluding majority non-domination in all three volatility regimes), traced to two concrete bugs -- the same real-corpus never-loaded wiring bug found independently in the new real-data run, and a PYTHONHASHSEED seed-irreproducibility bug affecting exactly the three stochastic policy families (AIMD, FreshCache, FreshCache-pooled). On convergence speed, the original hypothesis is now more firmly disconfirmed with confidence intervals rather than just point estimates: AIMD's median low-repeat convergence-event count (12.0-16.0, CIs up to [10.0,27.0] in high volatility) remains slower than d-TTL (11.0-12.0), EWMA (7.0-8.0), and FreshCache's raw 5.0-event figure, even though FreshCache's calibrated fraction is a tightly-bounded, genuinely low 0.29-0.41 across regimes (not just a single self-reported 0.375) -- and a new spot-check-rate ablation mechanistically confirms WHY AIMD is slow: convergence is gated by spot-check density, not by any intrinsic sluggishness in the AIMD update rule itself, since raising the spot-check rate closes most of AIMD's hit-rate gap without materially raising its stale rate. Two new methodological caveats, raised by review, now qualify the real-data dominance claim specifically and must be resolved before it can be treated as robust: (1) the real-data grid gave AIMD 9 knob points but every baseline only 3-5, an asymmetric search budget that could overstate the AIMD-vs-FreshCache gap in either direction; (2) FreshCache's real-data win has not been broken out by call-site repeat count, so it is untested whether the dominance survives in the low-repeat-count regime the paper itself identifies (via the convergence analysis and the real corpus's own median-5-revisit statistic) as both diagnostically important and most representative of real agent-tool-call sites, precisely where a model needing >=5 observations to fit should be weakest.\",\n  \"motivation\": \"The strongest existing adaptive-TTL result, d-TTL/f-TTL from CDN caching research, provably converges a TTL parameter toward a target *hit rate* using stochastic approximation -- but hit rate says nothing about whether those hits returned correct, non-stale content; it was built for content-delivery traffic where staleness usually isn't safety-relevant. The strongest existing staleness-aware caches for LLM pipelines, FreshCache (risk-constrained temporal caching for RAG) and vCache (verified semantic caching with Bayesian per-entry error-rate guarantees), instead fit an explicit probabilistic staleness/error model per entry and gate reuse against a fixed error budget -- powerful, but they require a labeled calibration signal to fit that model before the guarantee is meaningful, and the model itself must be re-fit or re-trusted whenever workload volatility shifts. Agent loops were hypothesized to be exactly the setting where a third option -- a cheap, purely reactive, feedback-driven AIMD rule with no fitting step -- would be attractive precisely because many call sites are visited only a handful of times per episode. Two iterations of increasingly careful measurement (fixing a silent dataset-wiring bug, adding independent bootstrap statistics, and now flagging an asymmetric knob-grid and a missing repeat-count breakdown) have not yet found that advantage: on real content specifically, a fitted gate's point estimate wins on hit rate at comparable staleness, and AIMD is confirmed slower, not faster, to reach a stable operating point. The remaining open question this hypothesis now tracks is narrower and more falsifiable than before: does AIMD's real-data disadvantage survive (i) statistically rigorous confidence intervals and (ii) a fair, matched knob-search budget against FreshCache, and does it specifically survive restriction to the low-repeat-count regime where a fitted model has the least data to work with -- the one regime where AIMD's model-free reactivity should, in principle, have room to matter.\",\n  \"assumptions\": [\n    \"For a sample of cache-served tool calls we can obtain a ground-truth valid/stale label after the fact (by issuing a live re-query and diffing), giving the confirmed-hit / confirmed-stale feedback signal the AIMD rule consumes -- the same kind of feedback FreshCache and vCache also require to calibrate, so this is not an extra assumption relative to the strongest baselines\",\n    \"Each tool call site (function + argument signature) recurs enough times within and across simulated episodes for a per-site reuse window to have room to grow and shrink -- true for the intended targets (file re-reads, repeated/near-duplicate searches, repeated computations) but not for one-shot calls, which no caching scheme helps anyway\",\n    \"A simulated or replayed agent-loop workload with realistic call repetition and controllable underlying volatility (so ground-truth staleness events can be injected/observed) is constructible from real seed content (Wikipedia/SQuAD, Quora Question Pairs, Our World in Data) replayed through deterministic repetition templates, since real production agent traces are not directly accessible -- though this remains a proxy for actual agent behavior, not a substitute for it\",\n    \"Convergence speed (staleness events needed before the policy stabilizes near a good operating point) is a meaningful axis to compare, not just the converged operating point itself -- appropriate given agent episodes are short relative to CDN traffic streams, so how fast a policy adapts matters as much as where it converges\",\n    \"A knob-search budget that is matched (or explicitly justified as saturating) across all compared policy families is necessary for a dominance claim between them to be trustworthy -- an asymmetric grid (9 points for AIMD vs 3-5 for baselines) is not a neutral simplification and must be corrected or justified before the real-data dominance finding can be treated as decisive\"\n  ],\n  \"investigation_approach\": \"Build an agent-loop tool-call harness (real-content-grounded versioned resource corpus replayed through deterministic repetition templates, or an OpenRouter-backed live-agent variant as a stretch goal) that logs every tool call, arguments, timestamp, and result, and can force a live re-query or ground-truth version-schedule lookup on a sampled subset of cache hits to get a binary valid/stale label. Implement cache policies at each call site: (1) fixed TTL swept over several values; (2) a d-TTL-style stochastic-approximation policy and an EWMA-adaptive policy that adapt TTL toward a target hit rate; (3) the proposed AIMD policy -- reuse window w_i per call site i, additive increase after each confirmed-valid hit, multiplicative decrease after each confirmed-stale hit, with w_i floor/ceiling bounds; and (4) a FreshCache-style fitted staleness-probability gate (raw and resource-class-pooled variants) as an upper-reference. Critically, next iteration must: (i) run the already-built bootstrap-CI/Holm-correction evaluation machinery against the real-data replicate rows (20 seeds/cell already exist) to attach confidence intervals and significance tests to the real-data dominance claim, not just the synthetic re-derivation; (ii) widen the real-data baseline knob grids to match AIMD's 9-point grid, or explicitly justify why the narrower grids already saturate each baseline's achievable frontier; (iii) break out Table-1-style hit-rate/stale-rate figures by call-site repeat-count bucket (low-repeat <=5 visits, matching the existing convergence-analysis bucketing) to test whether FreshCache's real-data dominance holds, narrows, or reverses specifically where a fitted model has the least data to calibrate on. Continue reporting the previously-established convergence-event and spot-check-density-ablation findings as settled (they are now confidence-interval-backed and mechanistically explained), and scope any novelty claims explicitly to caching literature unless an additional adjacent-domain (rate-limiting/autoscaling) search angle is added.\",\n  \"success_criteria\": \"CONFIRMS the original hypothesis's frontier claim only if, once the real-data dominance result is independently bootstrap-CI'd and significance-tested with a matched (or justified) knob-search budget across all policy families, AIMD's frontier position is NOT statistically dominated by FreshCache in the low-repeat-count bucket specifically (the regime the hypothesis's mechanism argues should favor a model-free reactive rule). DISCONFIRMS/refutes the frontier claim if, under those corrected conditions, FreshCache's dominance survives statistically and holds (or worsens) in the low-repeat bucket -- this would mean the fitted-gate approach beats AIMD even in exactly the regime AIMD was expected to have an edge, a strong and specific negative result. The convergence-speed claim (criterion b) is treated as already DISCONFIRMED with reasonable confidence given two independent lines of evidence (this iteration's CI-backed re-derivation and the mechanistic spot-check-density ablation); it is retained in the hypothesis only as a settled negative finding to report, not as an open question for further testing, unless a modified AIMD schedule (e.g., decaying additive increase, as noted in future work) is tested and shown to close the gap.\",\n  \"related_works\": [\n    \"FreshCache: Risk-Constrained Freshness-Aware Semantic Caching for Open-Web RAG (Mansoor, Ahmad & Yoon, arXiv:2607.04281, 2026) -- read in full. Fits an exponential-decay-plus-MLP staleness-probability model per entry/tier and gates reuse against a fixed per-tier error budget (0.10/0.20/0.35), evaluated on 8,072-31,201 real web queries with ground-truth snapshot labels. This is the closest prior mechanism -- probabilistic risk-budget gating -- and this iteration's real-data replay found it Pareto-dominates AIMD as a point estimate on real content, though that dominance is not yet independently statistically confirmed and has not been checked in the low-repeat-count regime where FreshCache's >=5-observation fitting requirement should be weakest.\",\n    \"vCache: Verified Semantic Prompt Caching (Schroeder et al., arXiv:2502.03771, 2025/2026) -- an online Bayesian learning algorithm that estimates a per-prompt-embedding similarity threshold to give user-defined error-rate guarantees on whether a semantically-matched cache hit is the *correct* answer; this targets match-correctness for semantic similarity caching, not time-based staleness of a fixed call's result, and still requires online posterior fitting per cached item, unlike the proposed reactive per-site window.\",\n    \"Adaptive TTL-Based Caching for Content Delivery -- d-TTL and f-TTL (Basu, Sundarrajan, Ghaderi, Shakkottai & Sitaraman, arXiv:1704.04448 / IEEE, 2017) -- read in full. d-TTL uses stochastic approximation (actor-critic style) to converge a per-object TTL toward a *target hit rate*, with provable convergence for bursty non-stationary CDN traffic. Neither d-TTL nor its EWMA-adaptive variant optimizes for confirmed correctness/staleness, and on the real-content replay both were matched or beaten by AIMD on hit rate at comparable staleness -- the frontier gap that remains is specifically against the fitted FreshCache gate, not against hit-rate-targeted adaptation.\",\n    \"ToolCacheAgent: Accelerating LLM Agent Through Intelligent Tool Call Caching (OpenReview 2026) -- an LLM-driven planner agent that assigns each tool a caching plan (cacheable / TTL / inter-tool invalidation rule) once, from the tool's semantics, reporting up to 1.69x latency speed-up; the plan is static once generated and not updated from observed confirmed-staleness feedback during execution, unlike the proposed per-site online-adapting window.\",\n    \"TVCACHE: A Stateful Tool-Value Cache for Post-Training LLM Agents (Vijaya Kumar et al., arXiv:2602.10986, 2026) -- caches by exact longest-prefix match on the agent's full tool-call-history tree, aimed at RL post-training rollouts with high trajectory overlap; this is an exact-match structural cache with no notion of graded time-based staleness or per-entry reuse window at all, addressing a different regime than the present hypothesis's within-episode, non-identical-trajectory redundant calls.\",\n    \"Concur: Proactive Agent-Level Admission Control for Efficient Agentic Batch Inference (Chen et al., ICML 2025, arXiv:2601.22705) -- applies genuine two-sided AIMD inside an LLM-agent inference-serving system, but to admission control of concurrent agents gated on KV-cache pressure, never to any object's TTL or content freshness; establishes AIMD as a live pattern in this research community for a categorically different control variable.\",\n    \"ClepsydraCache (Thoma et al., USENIX Security 2023, arXiv:2104.11469) and the Alex filesystem / RFC 7234 heuristic freshness rule -- the two closest non-agent, non-LLM near-hits for AIMD-shaped or age-proportional TTL adaptation, confirmed via a systematic literature search across CDN, database, DNS, and browser/HTTP caching literature to be triggered by security conflicts or resource age respectively, not confirmed content staleness; no hit was found applying a loss-event-triggered AIMD rule to per-object TTL in any of these domains, though the search did not extend to adjacent control-theoretic domains (rate-limiting, autoscaling/admission-control) where AIMD-style window adaptation is also an established pattern.\"\n  ],\n  \"inspiration\": \"PROCEDURAL/METHODOLOGICAL: TCP congestion control's AIMD rule was designed for exactly this class of problem -- adapt a resource-usage window under noisy, sparse, delayed feedback about an unknown and shifting environment, without ever fitting an explicit model of that environment, and recover fast from a bad outcome (packet loss / here, a confirmed-stale serve) via a sharp multiplicative cut while probing for more headroom via slow additive growth when things go well. That asymmetry -- cheap to gain, expensive to lose -- remains the shape wanted for cache reuse windows once staleness is reframed as a 'loss event', even though two iterations of increasingly rigorous measurement have not yet found it beating a fitted probabilistic gate. What now motivates continuing the investigation, rather than abandoning it, is a genuinely open, narrower question: the gap so far is measured only as a point estimate, with an unfair knob-search budget, and pooled across all repeat-count regimes including ones where a fitted model should have every advantage -- the fair, statistically confirmed, low-repeat-specific comparison has simply not been run yet.\",\n  \"terms\": [\n    {\n      \"term\": \"AIMD (additive increase, multiplicative decrease)\",\n      \"definition\": \"A feedback control rule, the core of TCP congestion control, that slowly grows a resource window on success and sharply shrinks it on a detected failure/overload signal, converging toward an efficient operating point without needing a model of the underlying system.\"\n    },\n    {\n      \"term\": \"Cache reuse window\",\n      \"definition\": \"The time interval, maintained per tool-call site, during which a cached tool result is served instead of re-executing the call; here it grows or shrinks based on confirmed-valid or confirmed-stale outcomes rather than being fixed or fit from a probability model.\"\n    },\n    {\n      \"term\": \"Confirmed-stale event\",\n      \"definition\": \"An observed instance where a cache hit was served but a live re-query (or later ground truth) shows the cached value no longer matches -- the 'loss signal' the AIMD rule reacts to, analogous to a dropped packet in TCP.\"\n    },\n    {\n      \"term\": \"Reduction-vs-staleness frontier\",\n      \"definition\": \"The tradeoff curve between how many redundant tool calls a caching policy avoids (efficiency) and how often it serves an outdated answer (correctness risk); comparing policies means comparing where each one lands on, and how it sweeps, this curve.\"\n    },\n    {\n      \"term\": \"Point-estimate dominance vs. statistically confirmed dominance\",\n      \"definition\": \"A distinction introduced this iteration: one policy's mean metrics beating another's across replicate seeds (point-estimate dominance) is not the same claim as that gap surviving bootstrap confidence intervals and significance testing (statistically confirmed dominance) -- the real-data FreshCache-over-AIMD result is currently only the former.\"\n    },\n    {\n      \"term\": \"Low-repeat-count bucket\",\n      \"definition\": \"Call sites visited five or fewer times per episode, matching the real corpus's own median revisit count; the regime the hypothesis identifies as both most representative of real agent-tool-call traffic and most disadvantageous for a fitted model like FreshCache that needs several observations to calibrate.\"\n    }\n  ],\n  \"summary\": \"We hypothesized that a TCP-congestion-control-style AIMD rule for per-call-site cache reuse windows matches or beats both fixed TTL and hit-rate-targeted adaptive TTL on the reduction-vs-staleness tradeoff while converging faster than a fitted probabilistic staleness gate. Two iterations of increasingly rigorous measurement instead found AIMD's convergence-speed advantage confidently disconfirmed (CI-backed, mechanistically explained by spot-check density) and its frontier position beaten by a fitted gate on real content -- but only as an unverified point estimate, with an unfair knob-search budget, and never yet checked in the specific low-repeat-count regime the mechanism was supposed to favor. The hypothesis now narrows to that unresolved, fair, statistically-confirmed, repeat-count-stratified comparison.\",\n  \"relation_type\": \"evolution\",\n  \"relation_rationale\": \"Same AIMD-vs-baselines frame; frontier claim narrowed to unverified-point-estimate + low-repeat gap per review\",\n  \"confidence_delta\": \"decreased\",\n  \"key_changes\": [\n    \"Reframed the real-data dominance finding (Table 1) from an established result to explicitly unverified point-estimate dominance, per reviewer MAJOR feedback that it lacks the bootstrap CIs the paper's own Section 5 demands of others\",\n    \"Added the asymmetric knob-search-budget caveat (9 points for AIMD vs 3-5 for baselines) as a first-class methodological gap that must be closed before the dominance claim is trusted, per reviewer MINOR feedback\",\n    \"Added the missing low-repeat-count breakdown as the single most important next test, since the hypothesis's own mechanism argues AIMD should have its best chance exactly where FreshCache has the least data to fit on -- currently untested\",\n    \"Retained and hardened the convergence-speed disconfirmation (now CI-backed across two independent artifacts plus a mechanistic spot-check-density ablation) as a settled negative finding rather than an open question\",\n    \"Softened novelty-search language to scope explicitly to caching literature, per reviewer MINOR feedback that adjacent control-theoretic domains (rate-limiting, autoscaling) were not searched\",\n    \"Investigation approach now prioritizes re-running existing evaluation machinery on existing real-data replicate rows (no new data collection needed) plus a matched-grid rerun, rather than proposing new experiment infrastructure\"\n  ],\n  \"artifact_relations\": [\n    {\n      \"from_id\": \"art_T0onLH9xokqw\",\n      \"to_id\": \"art_tceB4eOwcBAO\",\n      \"relation_type\": \"uses\",\n      \"relation_rationale\": \"Loads the real-content versioned corpus via a fail-fast dependency loader for the real-data replay\"\n    },\n    {\n      \"from_id\": \"art_qtEtMpLZuRGI\",\n      \"to_id\": \"art_tXld0p2SGjtU\",\n      \"relation_type\": \"extends\",\n      \"relation_rationale\": \"Re-imports method.py, reproduces its simulator/seeds, and adds bootstrap CIs and Holm-corrected tests\"\n    },\n    {\n      \"from_id\": \"art_T0onLH9xokqw\",\n      \"to_id\": \"art_tXld0p2SGjtU\",\n      \"relation_type\": \"uses\",\n      \"relation_rationale\": \"Uses corpus version-schedule stats for the ecological-validity proxy comparison\"\n    }\n  ]\n}\n", "structuredPatch": [], "originalFile": null, "userModified": false}
```

### [6] ASSISTANT · 2026-08-10 04:56:33 UTC

```
Done. The revised hypothesis narrows the frontier claim to unverified point-estimate dominance by FreshCache (pending bootstrap CIs and a matched knob grid), flags the untested low-repeat-count regime as the key remaining question, and retains the now CI-backed disconfirmation of AIMD's convergence-speed advantage. Output written to `.terminal_claude_agent_struct_out.json`.
```
